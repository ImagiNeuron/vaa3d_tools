...
8. `Simulate Somas` - Must be run after the `Soma Segmentation` function. Generates sythnetic somas in a synthetic background, along with a corresponding binary segmentation. Outputs these as images, along with a csv file containing their PCA results, a marker file locating synthetic somas, and a visualization of the simulation results.

## Architecture

The initial code for this plugin was generated with the `_Vaa3D_plugin_creator` plugin. The following contains a brief description of all the files that contain C++ code, and the build files.
