...
const double default_threshold_global =
    20;  // a small enough value for the last resort;
    ...
    bool control_run(unsigned char *_Image1D_original, V3DLONG _dim_X,
      ...
      for (V3DLONG idx_exemplar = 0; idx_exemplar < count_exemplar;
        ...
        if ((idx_step < 1) || (value_centerMovement2 > max_movment2) ||
            (poss_exemplarRegionOld.size() <
             default_threshold_regionSize))  // try again using loosened
                                             // criteria;
        {
          for (idx_step = 0; idx_step < count_step; idx_step++) {
            V3DLONG threshold_exemplarRegion = value_exemplar - idx_step;
            ...
            value_centerMovement2 =
                this->getEuclideanDistance2(pos_exemplar, pos_massCenterNew);
            if (value_centerMovement1 > (max_movment1 * 9)) {
              break;
            }
            if (value_centerMovement2 > (max_movment2 * 9)) {
              break;
            }
            pos_massCenterOld = pos_massCenterNew;
            poss_exemplarRegionOld = poss_exemplarRegionNew;
        ...
        for (V3DLONG idx_seed = 0; idx_seed < count_seed; idx_seed++) {
          ...
          for (V3DLONG idx_exemplar = 0; idx_exemplar < count_exemplar;
            ...
            V3DLONG threshold_backgroundValue =
                thresholds_voxelValue[idx_exemplarMapped];
            double threshold_valueChangeRatio =
                thresholds_valueChangeRatio[idx_exemplarMapped];
            V3DLONG threshold_regionSize =
                thresholds_regionSize[idx_exemplarMapped];
            if (value_seed < threshold_backgroundValue) {
              break;
            }
            V3DLONG uThreshold_regionSize =
                uThresholds_regionSize[idx_exemplarMapped];
            V3DLONG threshold_radius = thresholds_radius[idx_exemplarMapped];
            vector<V3DLONG> poss_region = this->regionGrowOnPos(
                pos_seed, threshold_backgroundValue, threshold_valueChangeRatio,
            ...
            if (size_radius <
                (threshold_radius * this->multiplier_thresholdRegionSize)) {
              for (V3DLONG idx_exemplar = 0; idx_exemplar < count_exemplar;
                   idx_exemplar++) {
                this->poss2Image1D(poss_region, masks_page[idx_exemplar], 0);
              }
              break;
            } else if (size_radius > (threshold_radius *
                                      this->multiplier_uThresholdRegionSize)) {
              continue;
            }
            V3DLONG pos_center = this->getCenterByMass(poss_region);
            vector<V3DLONG> xyz_center = this->index2Coordinate(pos_center);
            V3DLONG x = V3DLONG(xyz_center[0]);
            V3DLONG y = V3DLONG(xyz_center[1]);
            V3DLONG z = V3DLONG(xyz_center[2]);
            vector<vector<double> > valuesVct_regionShapeStat =
                this->getShapeStat(
                    x, y, z,
                    size_radius);  // consisted of 3 vectors with length 4;
            ...
            for (int m = 0; m < 4; m++) {
              ...
              if (fabs(values_PC1[m] - value_anisotropy) >
                  (this->threshold_deltaShapeStat * value_anisotropy)) {
                is_passedShapeTest = false;
                break;
              }
              value_anisotropy =
                  valueVctVct_exemplarShapeStat[idx_exemplarMapped][1][m];
              if (fabs(values_PC2[m] - value_anisotropy) >
                  (this->threshold_deltaShapeStat * value_anisotropy)) {
                is_passedShapeTest = false;
                break;
              }
              ...
              if (fabs(values_PC3[m] - value_anisotropy) >
                  (this->threshold_deltaShapeStat * value_anisotropy)) {
                is_passedShapeTest = false;
                break;
              }
...
#pragma region "regionGrow"
    void categorizeVoxelsByValue()  // will only consider voxels with value
                                    // higher than threshold_global;
    {
        ...
        for (int j = 0; j < const_count_neighbors; j++) {
          if (((xyz_current[0] + point_neighborRelative[j].x) < 0) ||
            ...
            if (this->checkValidity(
                    pos_neighbor))  // prevent it from going out of bounds;
            {
